          ...
          <div key={section.titre}>
            ...
            <div>
              {section.membres.map((membre) => {
                ...
                const estVous = currentUserId !== undefined && membre.id === currentUserId;
                const estAffecte = (membresAffectesIds ?? []).includes(membre.id);
                return (
                  <button
                    key={membre.id}
                    type="button"
                    onClick={() => onTogglePriseDeService(membre.id)}
                    className={`flex w-full items-center gap-2.5 border-b border-panel-border/50 border-l-2 px-4 py-2 text-left outline-none transition-colors hover:bg-panel-bg focus-visible:bg-panel-bg ${
                      enService ? "bg-panel-accent/10" : ""
                    } ${estVous ? "border-l-panel-accent" : "border-l-transparent"}`}
                  >
                    <span className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-panel-border text-xs font-semibold text-panel-text">
                      ...
                      {enService && (
                        <span
                          className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-panel-surface ${
                            estAffecte ? "bg-amber-500" : "bg-emerald-500"
                          }`}
                        />
                      )}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className="flex items-center gap-1.5">
                        ...
                        {estVous && (
                          <span className="shrink-0 rounded bg-panel-accent/20 px-1 text-[10px] text-panel-accent">
                            Vous
                          </span>
                        )}
                        {enService && estAffecte && (
                          <span className="shrink-0 rounded bg-amber-500/15 px-1 text-[10px] text-amber-400">
                            Affecté
                          </span>
                        )}
                      </span>
